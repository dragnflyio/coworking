...
class Validation{
	...
	function getMemberPackage($memberid){
		$ret = array();
		if ($row = $this->em->fetchAssoc('SELECT packageid, p.name AS packagename FROM member_package mp JOIN package p ON mp.packageid = p.id WHERE memberid = ? AND 1 = mp.active', array($memberid))){
			$ret = $row;
		}
		return $ret;
	}
